...
const countListFAQ = `-- name: CountListFAQ :one
select count(*)
from faq
...
func (q *Queries) CountListFAQ(ctx context.Context) (int64, error) {
	row := q.queryRow(ctx, q.countListFAQStmt, countListFAQ)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) GetFAQ(ctx context.Context, id int64) (GetFAQRow, error) {
	...
	var i GetFAQRow
	err := row.Scan(&i.Question, &i.Answer)
	return i, err
}
...
func (q *Queries) ListFAQ(ctx context.Context, arg ListFAQParams) ([]ListFAQRow, error) {
	rows, err := q.query(ctx, q.listFAQStmt, listFAQ, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
